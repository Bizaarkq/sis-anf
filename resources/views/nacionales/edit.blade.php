  ...
  <div class="flex items-center justify-center p-12">
    <div class="w-full max-w-2xl mx-auto bg-white shadow-lg rounded-sm border border-gray-200">
      ...
      <div class="mx-auto w-full max-w-[550px]">
        <form action="{{route('nacionales.update', ['id' => $ratio->ID_RATIO_POR_TIPO])}}" method="PUT">
          @csrf
          <div class="row mb-3">
          ...
          <select class="form-select"  name="catalogo">
            ...
            <option value="{{$ratios->ID_RATIO_CATALOGO}}" id="{{$ratios->ID_RATIO_CATALOGO}}" {{$ratio->NOMBRE_RATIO_CATALOGO === $ratios->NOMBRE_RATIO_CATALOGO ? 'selected' : ''}}>
          ...
          <select class="form-select"  name="sectores">
            ...
            <option value="{{$rubros->ID_TIPO_SECTOR}}" id="{{$rubros->ID_TIPO_SECTOR}}" {{$ratio->NOMBRE_TIPO_SECTOR === $rubros->NOMBRE_TIPO_SECTOR ? 'selected' : ''}}>
          ...
          <input required type="number" step="0.01" id="VALOR_RATIO_POR_TIPO"
              name="VALOR_RATIO_POR_TIPO" value="{{$ratio->VALOR_RATIO_POR_TIPO}}"> 
          <div class="row mb-3">
          </div>
          <div class="row mb-3">
          </div>  
